            ...
            <main className="pt-24 px-6 max-w-6xl mx-auto pb-20">
                <div className="flex justify-between items-center mb-10">
                    <h1 className="text-4xl font-black tracking-tight flex items-center gap-3">
                        ...
                        ADMIN HUB
                        <span className="text-[10px] font-black bg-white/5 border border-white/10 px-2 py-1 rounded text-primary uppercase ml-2">MASTER</span>
                    </h1>
                    <button
                        onClick={async () => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                    <Link href="/kitchen" className="bg-[#181818] p-6 rounded-2xl border border-white/5 hover:border-blue-500/50 transition-all flex items-center gap-4 group">
                    ...
                    <Link href="/admin/products" className="bg-[#181818] p-6 rounded-2xl border border-white/5 hover:border-purple-500/50 transition-all flex items-center gap-4 group">
                    ...
                    <Link href="/admin/categories" className="bg-[#181818] p-6 rounded-2xl border border-white/5 hover:border-orange-500/50 transition-all flex items-center gap-4 group">
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-3xl p-8 mb-16 flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex gap-4">
                        <Link href="/admin/history" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-white px-6 py-4 rounded-2xl font-bold transition-all border border-white/10">
                        ...
                        <button
                            onClick={async () => {
                                if (confirm('START NEW DAY?\n\nThis will:\n1. Close ALL active tables\n2. Archive today\'s revenue to History\n3. Reset the dashboard for a fresh start.\n\nARE YOU SURE?')) {
                                    const res = await fetch('/api/admin/start-new-day', { method: 'POST' });
                        ...
                        {activeSessions.map((session) => (
                            <div key={session.id} className="bg-[#181818] rounded-2xl p-6 border-2 border-primary/20 shadow-xl overflow-hidden relative group transition-all hover:scale-[1.02]">
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        <h3 className="text-3xl font-black">TABLE {session.tableId}</h3>
                        ...
                        <div className="bg-white text-black w-full max-w-md rounded-3xl overflow-hidden shadow-2xl transform transition-all animate-in fade-in zoom-in duration-200">
                            <div className="p-8 pb-4">
                                <div className="flex justify-between items-start mb-8">
                                    <div>
                                        <h2 className="text-3xl font-black tracking-tighter">BROASTIFY</h2>
