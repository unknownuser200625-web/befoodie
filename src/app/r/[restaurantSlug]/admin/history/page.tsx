...
import { DailyHistory, Restaurant, Order, TableSession, OrderItem } from '@/types';
            ...
            <main className="pt-24 px-6 max-w-5xl mx-auto pb-20">
                <div className="flex items-center gap-4 mb-10">
                    ...
                    <h1 className="text-4xl font-black tracking-tight flex items-center gap-3 uppercase italic">
                        ...
                        {restaurant?.name || 'BEFOODIE'} | HISTORY
                    </h1>
                </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 grid md:grid-cols-2 gap-8">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {selectedDay.sessions.map((session: TableSession) => (
                                            <div key={session.id} className="bg-white/5 border border-white/5 p-5 rounded-2xl">
                                                ...
                                                <div className="space-y-1">
                                                    {selectedDay.orders.filter((o: Order) => o.sessionId === session.id).map((order: Order) => (
                                                        <div key={order.id} className="text-xs text-gray-400 flex justify-between">
                                                            <span>#{order.id.slice(-4)} • {order.items.map((i: OrderItem) => `${i.quantity}x ${i.name}`).join(', ')}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
